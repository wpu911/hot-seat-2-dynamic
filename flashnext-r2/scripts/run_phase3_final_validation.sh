...
require_alias "$PROD_ALIAS" || { echo "ERROR production alias missing" >&2; exit 4; }
require_alias "$FINAL_ALIAS" || { echo "ERROR final candidate alias missing" >&2; exit 5; }
...
python3 "$SCRIPT_DIR/bench_llamaswap_ab.py" \
  --baseline "$PROD_ALIAS" \
  --r2 "$FINAL_ALIAS" \
  --rounds "${SHORT_ROUNDS:-4}" \
  --repeat "${SHORT_REPEAT:-3}" \
  --tg "${SHORT_TG:-512}" \
  --pp "${SHORT_PP:-512,2048,8192}" \
  --out "$SHORT_JSON"
...
# 2) Full long-context ladder. The analyzer is used as a correctness/regression
# gate here, not to force a QSA speedup when QSA was rejected in Phase-2.
...
# 3) Cached Large-PP / high-LCP branch. This catches the old 0.1 tok/s family of
# regressions that ordinary fresh prompts happily miss.
