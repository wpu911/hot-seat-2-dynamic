...
# Flash Next R2 Stage 8: incremental pooled-key cache for the qwen4exp QSA indexer.
# Stack order:
#   exact production HEAD
#     + PR #28213 QSA gather
#     + PR #28699 incremental pooled-key cache
#
# OFF and ON aliases use the SAME binary and QSA gather is ON on both sides.
# The only Stage-8 variable is LLAMA_QSA_NO_POOLED_CACHE:
#   OFF: variable present -> pooled cache disabled
#    ON: variable absent  -> pooled cache enabled
#
# Production source/runtime/alias are never modified in place.
...
if [[ -e "$R2_SRC" ]]; then
  ...
  echo "Refusing to overwrite an experiment tree." >&2
  exit 3
fi
if [[ -e "$RUNTIME" ]]; then
...
# A long-context QSA experiment is meaningless if TOP_K silently falls back to CPU.
if ! grep -RqiE 'radix.*top.?k|top.?k.*radix|radix_select|top_k_radix' "$PROD_SRC/ggml/src/ggml-cuda"; then
  echo "ERROR: ROCm long-row radix TOP_K not confirmed in production source." >&2
...
fetch_patch() {
  ...
  if [[ ! -f "$dst" ]]; then
    curl -fL --retry 3 --connect-timeout 20 "$url" -o "$dst"
  fi
...
# Commit each functional port inside the detached experiment tree so the second
# 3-way apply sees the first patch as a real base instead of a dirty index.
apply_and_commit() {
  local patch="$1" msg="$2"
  if ! git apply --3way "$patch"; then
    echo "ERROR: failed to apply $patch" >&2
    echo "Worktree is intentionally kept for manual functional migration." >&2
...
if ! grep -Rq 'LLAMA_QSA_NO_POOLED_CACHE' src; then
  echo "ERROR: pooled-cache kill switch missing after patch stack" >&2
  exit 11
fi
if ! grep -Rq 'QWEN4EXP_QSA_GATHER' src; then
  echo "ERROR: QSA gather kill switch missing after patch stack" >&2
  exit 12
fi
...
# Same binary, same model arguments, QSA gather ON on both sides.
# OFF: presence of LLAMA_QSA_NO_POOLED_CACHE disables the new cache.
...
# ON: do NOT set LLAMA_QSA_NO_POOLED_CACHE. Its absence is the enable state.
python3 "$SCRIPT_DIR/install_llamaswap_r2_alias.py" \
  --alias "$ON_ALIAS" \
  --r2-bin "$RUNTIME" \
  --jmax 0 \
