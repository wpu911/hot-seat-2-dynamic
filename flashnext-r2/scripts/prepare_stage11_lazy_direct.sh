...
#
# Correct baseline for R2 is the already validated Modern-MTP tree, not Sep-11
# production. Both A/B aliases use the SAME candidate binary; only --lazy-mode
# differs:
#   mmap   = on
#   direct = on-direct
#
# This keeps the experiment orthogonal to the newer qwen4exp/MTP engine and
# prevents us from measuring a fast PLE path on an obsolete runtime.

BASE_SRC="${BASE_SRC:-/app/share/llama_box/src/llama.cpp-flashnext-r2-modern-mtp-20260918}"
R2_SRC="${R2_SRC:-/app/share/llama_box/src/llama.cpp-flashnext-r2-modern-lazy-direct-20260918}"
RUNTIME_ROOT="${RUNTIME_ROOT:-/app/share/llm/Qwen3.8-Flash-Next-GGUF/runtime-text/r2-modern-lazy-direct}"
...
MMAP_ALIAS="${MMAP_ALIAS:-qwen3.8-flash-next-r2-modern-lazy-mmap:256k}"
DIRECT_ALIAS="${DIRECT_ALIAS:-qwen3.8-flash-next-r2-modern-lazy-direct:256k}"
...
[[ -e "$BASE_SRC/.git" ]] || {
  echo "ERROR: modern-MTP source missing: $BASE_SRC" >&2
  echo "Run prepare_stage10_upstream_mtp.sh first." >&2
  exit 2
}
[[ -z "$(git -C "$BASE_SRC" status --porcelain)" ]] || {
  echo "ERROR: modern-MTP base must be clean/committed." >&2
  exit 3
}
[[ -f "$BASE_SRC/r2-meta/stage10-modern-mtp-base.txt" ]] || {
  echo "ERROR: source is not a recorded Stage-10 modern-MTP tree: $BASE_SRC" >&2
  exit 4
}
[[ ! -e "$R2_SRC" && ! -e "$RUNTIME_ROOT" ]] || {
  echo "ERROR: Stage-11 source/runtime already exists; refusing overwrite" >&2
...
# Inspect the modern-MTP alias before building. PR #29030 compares lazy mmap
# with direct reads. If the inherited production/model configuration forces
# --no-mmap, that is a different experiment and we stop instead of producing
# ceremonial benchmark numbers.
BLOCK="$(python3 - "$CONFIG" "$SOURCE_ALIAS" <<'PY'
import re, sys
p, alias = sys.argv[1:]
...
raise SystemExit(3)
PY
)" || { echo "ERROR: source alias not found: $SOURCE_ALIAS" >&2; exit 7; }

if grep -qE '(^|[[:space:]])--no-mmap([[:space:]]|$)' <<<"$BLOCK"; then
  echo "ERROR: modern-MTP Flash Next alias explicitly contains --no-mmap." >&2
  echo "PR #29030 lazy mmap/direct A/B is not valid on this configuration." >&2
  exit 8
fi

echo "=== source alias lazy-mode hints ==="
...
echo "=== Stage 11 / modern lazy-direct ==="
echo "modern-MTP source : $BASE_SRC"
echo "modern-MTP head   : $BASE_HEAD"
...
mkdir -p "$R2_SRC/r2-meta"
{
  ...
  echo "modern_mtp_source=$BASE_SRC"
  echo "modern_mtp_head=$BASE_HEAD"
...
} > "$R2_SRC/r2-meta/stage11-modern-lazy-direct-base.txt"
...
if ! git -c user.name='FlashNext R2 Experiment' \
         ...
         am --3way "$PATCH"; then
  echo >&2
  echo "ERROR: PR #29030 conflicts with the Modern-MTP + HotSeat tree." >&2
  echo "Candidate tree is intentionally kept for a semantic merge: $R2_SRC" >&2
  echo "Do not resolve loader/runtime conflicts with blanket --theirs." >&2
...
# Guard the features Stage 11 is not allowed to lose.
for needle in \
  'qwen4exp_shared_model' \
  'LLM_GRAPH_TYPE_DECODER_MTP' \
  'HOTSEAT'; do
  if grep -Rqs "$needle" src common ggml tools/server 2>/dev/null; then
    echo "PROBE_PRESENT $needle"
  else
    echo "ERROR: inherited feature disappeared while applying #29030: $needle" >&2
    exit 13
  fi
done

{
  ...
  echo "candidate_diff_sha256=$(git diff --binary "$BASE_HEAD" "$CANDIDATE_HEAD" | sha256sum | awk '{print $1}')"
} >> r2-meta/stage11-modern-lazy-direct-base.txt
...
BUILD="${BUILD:-$R2_SRC/build-r2-modern-lazy-direct}"
...
# Two runtime copies, byte-identical real binary/libraries. Wrapper strips any
# inherited lazy-mode option and injects exactly one mode, making the A/B honest.
make_runtime() {
  local dst="$1" mode="$2"
  mkdir -p "$dst"
  cp -a "$BUILD/bin/." "$dst/"
...
[[ "$MMAP_SHA" == "$DIRECT_SHA" ]] || { echo "ERROR: mmap/direct real binaries differ" >&2; exit 21; }

# Clone the validated Modern-MTP alias verbatim. Keep model path, draft path,
# HotSeat, Dynamic KV, MTP depth, split settings and JMAX identical.
...
echo
echo "Stage-11 modern lazy direct-read A/B is ready."
