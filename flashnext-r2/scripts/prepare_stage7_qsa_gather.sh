...
# The patch comes from an immutable commit URL; also verify it advertises the pinned commit.
...
# The merged ROCm long-row TOP_K support should already be in this Sep-11 tree.
# Verify before layering QSA gather; otherwise a CPU TOP_K fallback would poison
# the long-context A/B and make the result meaningless.
if ! grep -RqiE 'radix.*top.?k|top.?k.*radix|radix_select|top_k_radix' "$PROD_SRC/ggml/src/ggml-cuda"; then
  ...
  echo "Run verify_stage7_rocm_topk.sh and inspect before continuing." >&2
  exit 6
fi

# Isolated detached worktree from exact production HEAD.
...
if ! git apply --3way "$PATCH"; then
  echo "ERROR: PR #28213 did not apply cleanly to the production HotSeat tree." >&2
...
# Quick correctness gate for the backend operation that QSA depends on.
...
# Same binary, same cloned production model arguments. Only QSA gather differs.
