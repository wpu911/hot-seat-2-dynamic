#!/usr/bin/env python3
"""Analyze Flash Next R2 Stage-3 GDN A/B results.

GDN fusion is expected to be a smaller win than HyperConnection fusion, so the
default gain threshold is lower. Exact deterministic output remains mandatory.
"""
...
def main():
    ...
    ap.add_argument("--min-median-tg-gain", type=float, default=1.0)
    ...
    out = Path(args.result).with_suffix(".gdn-analysis.json")
